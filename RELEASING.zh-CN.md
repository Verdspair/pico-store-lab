...
2. 本地执行 SDK 测试和 Android 构建，并等待 CI 全绿；构建 Python wheel/sdist、TS 包、Rust crate 检查、Kotlin AAR 和 Android APK。区分用户先前的实机验证与本次尚待实机复验的修复。
...
5. 为已验证提交打 `vX.Y.Z` 标签并创建 GitHub Release，仅附 SDK/构建产物和校验值。发布说明明确区分已验证结果与限制。
