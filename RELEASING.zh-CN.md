...
1. 同步更新四套 SDK、Android 版本号、变更记录和中英文 README。
...
4. 应用远端 D1 迁移后用 Wrangler 部署 `apps/website`，核验 `https://pico.kanglives.top/` 和 `/api/releases`。公开接口只能包含元数据，不能含账号或下载数据。若推迟部署，需要在发布说明中说明。
...
MIT 只授权本项目代码，不授权再分发 PICO 或 VRChat APK。
